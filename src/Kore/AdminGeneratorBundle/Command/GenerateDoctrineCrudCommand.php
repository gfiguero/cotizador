...
use Kore\AdminGeneratorBundle\Generator\DoctrineCrudGenerator;
...
class GenerateDoctrineCrudCommand extends GenerateDoctrineCommand
{
    ...
    {
        $this
            ->setName('kore:admingenerate:crud')
            ->setDescription('Generates a CRUD form AdminBundle based on a Doctrine entity')
            ->setDefinition(array(
...
EOT
    ...
    {
        $adminBundle = $this->getContainer()->get('kernel')->getBundle('KoreAdminBundle');
        ...
        if (!$ret) {
            $help = sprintf("        <comment>resource: \"@KoreAdminBundle/Resources/config/routing/%s.%s\"</comment>\n", strtolower(str_replace('\\', '_', $entity)), $format);
